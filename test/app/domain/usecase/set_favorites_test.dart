...
void main() {
  ...
  late SetFavoritesUseCaseImp setFavoritesUseCaseImp;
  late MovieDetailsEntity movieDetailsEntity;

  setUp(() {
    favoritesRepository = SetMoviesRepositoryMock();
    setFavoritesUseCaseImp = SetFavoritesUseCaseImp(favoritesRepository);
...
group("[Domain] - SetFavoritesUseCase", () {
    test("Deve retornar sucesso", () async {
      when(() => favoritesRepository.setFavorite(movieDetailsEntity))
          .thenAnswer((invocation) async => const Right(true));

      final result = await setFavoritesUseCaseImp(movieDetailsEntity);
